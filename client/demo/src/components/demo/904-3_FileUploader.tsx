...
// Inside FileUploaderScreen component
...
export const FileUploaderScreen = (props: FileUploaderScreenProps) => {
  ...
  const [voiceChangerType, setVoiceChangerType] =
    useState<VoiceChangerType>("RVC");
  ...
  const screen = useMemo(() => {
    ...
    const generateFileRow = (
      setting: ModelUploadSetting,
      title: string,
      kind: ModelFileKind,
      ext: string[],
      dir: string = ""
          ...
          <div
            ...
            onClick={async () => {
              ...
              if (checkExtention(file.name, ext) == false) {
                const alertMessage = `${messageBuilderState.getMessage(
                  __filename,
                  "alert-model-ext"
                )} ${ext}`;
                alert(alertMessage);
    ...
    const generateFileRowsByVCType = (vcType: VoiceChangerType) => {
      ...
      if (vcType == "RVC") {
        ...
        rows.push(
          generateFileRow(uploadSetting!, "Config", "mmvcv13Config", ["json"])
        );
        rows.push(
          generateFileRow(uploadSetting!, "Model", "mmvcv13Model", [
            "pth",
            "onnx",
          ])
        );
        ...
        rows.push(
          generateFileRow(uploadSetting!, "Config", "mmvcv15Config", ["json"])
        );
        rows.push(
          generateFileRow(uploadSetting!, "Corres", "mmvcv15Correspondence", [
            "txt",
          ])
        );
        rows.push(
          generateFileRow(uploadSetting!, "Model", "mmvcv15Model", [
            "pth",
            "onnx",
          ])
        );
        ...
        rows.push(
          generateFileRow(uploadSetting!, "Config", "soVitsSvc40Config", [
            "json",
          ])
        );
        rows.push(
          generateFileRow(uploadSetting!, "Model", "soVitsSvc40Model", ["pth"])
        );
        rows.push(
          generateFileRow(uploadSetting!, "Cluster", "soVitsSvc40Cluster", [
            "pth",
            "pt",
          ])
        );
        ...
        rows.push(
          generateFileRow(uploadSetting!, "Model(combo)", "diffusionSVCModel", [
            "ptc",
          ])
        );
      } else if (vcType == "Beatrice") {
        rows.push(
          generateFileRow(uploadSetting!, "Beatrice", "beatriceModel", ["bin"])
        );
      } else if (vcType == "LLVC") {
        rows.push(
          generateFileRow(uploadSetting!, "Model", "llvcModel", ["pth"])
        );
        rows.push(
          generateFileRow(uploadSetting!, "Config", "llvcConfig", ["json"])
        );
      } else if (vcType == "EasyVC") {
        rows.push(
          generateFileRow(uploadSetting!, "Model", "easyVCModel", ["onnx"])
        );
      }
      return rows;
    };
    ...
    const buttonLabel =
      serverSetting.uploadProgress == 0
        ...
        : messageBuilderState.getMessage(__filename, "uploading") +
          `(${serverSetting.uploadProgress.toFixed(1)}%)`;
    return (
      <div className="dialog-frame">
        ...
        <div className="dialog-fixed-size-content">
          <div className="file-uploader-header">
            {messageBuilderState.getMessage(__filename, "header_message")} Slot[
            {props.targetIndex}]
            <span
              onClick={() => {
            ...
            <div
              ...
              onClick={() => {
                ...
                if (checkModelSetting(uploadSetting)) {
                  ...
                  const errorMessage = messageBuilderState.getMessage(
                    __filename,
                    "alert-model-file"
                  );
                  alert(errorMessage);
          ...
          >
            <div className="file-uploader-file-select-row-label">
              Download from URL (Applio Style):
            </div>
            <input
              type="text"
            ...
            <div
              className="file-uploader-file-select-upload-button"
              style={{ backgroundColor: isDownloading ? "#555" : "#e91e63" }} // Applio Pink
  ...
  const handleUrlDownload = async () => {
    ...
    formData.append("slot", props.targetIndex.toString()); // Use the current slot

    try {
      // This calls the Python endpoint we created earlier
      ...
      if (res.ok && data.status !== "error") {
        setDownloadStatus("Success! Model installed.");
        // Optional: Refresh or close the dialog
        setTimeout(() => props.backToSlotManager(), 1500);
      } else {
        setDownloadStatus("Error: " + (data.message || "Unknown error"));
      }
    } catch (e: any) {
